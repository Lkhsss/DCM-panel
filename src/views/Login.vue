<template>
  <div class="min-h-screen px-6 py-20 md:px-20 lg:px-80 flex items-center justify-center backdrop-blur-3xl"
    style="background-image: linear-gradient(135deg, #f472b6 0%, #3b82f6 30%, #ffffff 100%);">
    <div
      class="px-8 md:px-12 lg:px-20 py-12 flex flex-col items-center gap-12 w-full backdrop-blur-3xl rounded-2xl bg-white/20 border border-white/15 max-w-sm">
      <div class="flex flex-col items-center gap-4 w-full">
        ...
        <div class="flex flex-col gap-2 w-full">
          <div class="text-center text-3xl font-medium text-white leading-tight">风纪面板</div>
        ...
        <div class="flex flex-col gap-6 w-full">
          <IconField>
            ...
            <InputText type="text" v-model="username"
              class="appearance-none! border! border-white/10! w-full! outline-0! bg-white/10! text-white! placeholder:text-white/70! rounded-3xl! shadow-sm! transition-all! duration-200! ease-out! focus:scale-[1.03]! focus:shadow-[0_0_20px_rgba(59,130,246,0.45)]! focus:border-white/40! focus:ring-2! focus:ring-white/30!"
...
async function login(this: any) {
  ...
  try {
    ...
    if (response.status !== 200) {
      toast.add({ severity: 'custom', summary: '登录失败', detail: '无法登录!\n原因：' + response.data, life: 3000, styleClass: error_styleClass })
      return
    }

    toast.add({ severity: 'custom', summary: '登录成功', detail: '欢迎回来', life: 3000, styleClass: success_styleClass })
    // 跳转到主页
