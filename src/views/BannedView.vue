...
async function ban(id: number) {
    ...
    if (duration < 0) {
        toast.add({ severity: 'custom', summary: '封禁时间必须在当前时间之后', life: 3000, styleClass: error_styleClass })
    } else {
        ...
        if (response.status === 200) {
            if (response.data['status'] === 'Banned') {
                if (duration !== 0) {
                    toast.add({ severity: 'custom', summary: '封禁ID: ' + id, detail: "解禁时间：" + formatData(bannd_time.value), life: 3000, styleClass: success_styleClass })
                } else {
                    toast.add({ severity: 'custom', summary: '封禁ID: ' + id, detail: "永久封禁", life: 3000, styleClass: success_styleClass })
                }
            }
        } else if (response.status === 409) {
            toast.add({ severity: 'custom', summary: '封禁失败', detail: "该用户已被封禁", life: 3000, styleClass: error_styleClass })
...
onBeforeMount(async () => {
    ...
    if (totalRecords.value === 0) {
        toast.add({ severity: 'custom', summary: '当前没有数据', detail: "点击添加按钮添加或使用机器人封禁哦~" + totalRecords.value, life: 3000, styleClass: info_styleClass })
    }
...
<template>
    <DataTable :value="datalist" paginator :rows="rows" :rowsPerPageOptions="[10, 15, 20, 50]" :filters="filters"
        ...
        <Column field="duration" header="持续时间">
            <template #body="{ data }">
